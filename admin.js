...
document.getElementById("produtoForm").onsubmit = async e => {
  ...
  try {
    ...
    const urlsDigitadas = (document.getElementById("imagens").value || "")
      .split(",")
      .map(x => x.trim())
      .filter(Boolean);

    let principal = document.getElementById("imagem").value.trim();

    if (novasImagens.length) {
      principal = novasImagens[0];
    }

    const todas = novasImagens.slice(1).concat(urlsDigitadas);
